...
class VGG16mo:
    ...
    def max_pool_s4(self, bottom, name):
        return tf.nn.max_pool2d(input=bottom, ksize=[1, 2, 2, 1], strides=[1, 4, 4, 1], padding='SAME', name=name)

    def conv_layer(self, bottom, name):
        with tf.compat.v1.variable_scope(name):
            filt = self.get_conv_filter(name)

            conv = tf.nn.conv2d(bottom, filters=filt, strides=[1, 1, 1, 1], padding='SAME')
